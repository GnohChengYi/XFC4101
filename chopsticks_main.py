...
random.seed(4101)
num_trials = 10 if DEBUG else 10
methods = [KNNImputation(), MissForestImputation(), MiceImputation(), LinearInterpolator()] if DEBUG else [KNNImputation(), MissForestImputation(), MiceImputation(), LinearInterpolator()]
missing_fractions = [0.7] if DEBUG else [0.5, 0.6, 0.7, 0.8, 0.9]
...
ylabel = "Quality Rank"
title = f"Chopsticks, {int(missing_fraction * 100)}% missing data"
